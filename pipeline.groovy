...
def carriers = ['ups', 'ontrac','mock', 'ups04']

String testsCarrierUuid = 'ffffffff-0000-ffff-0000-ffffffffffff'

String provider = 'aws'
String product = 'microservices'
// display name on Jenkins
String serviceDisplayName = 'ZoneProcessor'
String basePath = 'metapackteam'

// docelowo dać generyczny:
String serviceRepoAddress = "git@bitbucket.org:metapack/microservices-zone-processor.git"

//path to jenkins files on serviceRepoAddress
String jenkinsFiles = "pipelines"
String serviceDockerRepoAddressDev = "metapackltd/zone-processor-core-dev"
String serviceDockerRepoAddressPrd = "metapackltd/zone-processor-core"
        ...
        for (customer in envConfig.value.customers) {
            ...
            if (envConfig.key == "dev" || envConfig.key == "int")
            {
                pipelineJob("${jobPath}/${serviceDisplayName}/${envConfig.key}/${region}/${customer}/integration-tests") {
                    ...
                    parameters {
                        stringParam('SERVICE_ENDPOINT', "http://prv.processor-${testsCarrierUuid}.zone.${region}.${envConfig.key}.aws.metapack.io", 'Address of the running service to be tested')
                ...
                pipelineJob("${jobPath}/${serviceDisplayName}/${envConfig.key}/${region}/${customer}/performance-tests") {
                    ...
                    parameters {
                        ...
                        if("${envConfig.value.awsAccount}" == "playground"){
                            stringParam('ENDPOINT', "http://prv.processor-${testsCarrierUuid}.zone.${region}.${envConfig.key}.aws.metapack.io", 'Endpoint')
